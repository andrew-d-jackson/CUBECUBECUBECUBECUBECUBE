...
impl WritableTextures {
 ...
 /*   pub fn get_frame_buffer(name: String, display: &Display) -> SimpleFrameBuffer {
    ...
    pub fn resize(&mut self, name: String, display: &Display) -> () {
        ...
        self.color_textures.insert(name.clone(), Arc::new(Mutex::new(color_texture)));
        self.depth_textures.insert(name.clone(), Arc::new(Mutex::new(depth_texture)));
    }
    ...
    pub fn get_textures(&self, name: String) -> (MutexGuard<Texture2d>, MutexGuard<DepthTexture2d>) {
        (
