use glium::{IndexBuffer, VertexBuffer, Display, implement_vertex};
...
impl TexturedModel {
    pub fn new(file: String, display: &Display) -> Self {
        ...
        for m in models.iter() {
            ...
            let index_buffer = IndexBuffer::new(display, PrimitiveType::TrianglesList, &mesh.indices.as_ref()).unwrap();

            let material = &materials[mesh.material_id.unwrap()];
            let img = image::open(format!("objs/{}", material.diffuse_texture)).unwrap().into_rgba();
            let img_dimensions = img.dimensions();
            let img_raw = glium::texture::RawImage2d::from_raw_rgba_reversed(&img.into_raw(), img_dimensions);
            let diffuse_texture = glium::texture::SrgbTexture2d::new(display, img_raw).unwrap();

            individual_models.push(IndividualModel { vertex_buffer, index_buffer, texture: diffuse_texture });
        }
        Self { models: Arc::new(Mutex::new(individual_models)) } 
    }
}